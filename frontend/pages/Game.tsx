...
const Game = () => {
  ...
  const [selectedNumbers, setSelectedNumbers] = useState<number[]>([7, 22, 54]);
  const [checkoutNumber, setCheckoutNumber] = useState<number | null>(null);

  const toggleNumber = (number: number) => {
    // Open checkout modal instead of selecting immediately
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-3 shadow-sm sm:p-4">
        <div className="mb-3 flex items-center justify-between px-1 sm:px-2">
          <div>
            <p className="text-xs font-medium text-slate-400">
              Bingo board
            </p>
            <h2 className="text-lg font-semibold text-white">100 numbers</h2>
          </div>
          <button
            type="button"
